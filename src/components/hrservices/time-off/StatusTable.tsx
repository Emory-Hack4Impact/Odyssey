...
// Import TimeOffRequest type from where it's defined (TimeOffForm or a common types file)
import { type TimeOffRequest } from "./TimeOffForm"; 

// Helper to format the Date object to a readable string (MM/DD/YYYY)
        ...
        {requests.length === 0 ? (
          ...
          requests.map((request) => (
            <tr key={request.id}>
              ...
              <td className="border-b border-gray-200 px-4 py-2">
                {/* Displaying 'Pending' for all new requests since 'approved' is false */}
